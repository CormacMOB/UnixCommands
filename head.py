...
"""
def print_bytes(Filename,number_of_bytes):
# opens a file, reads the number of bytes from the file OR all but the last number of bytes.
    source = open(Filename, 'r')
    if number_of_bytes > 0:
        data_to_print = source.read(number_of_bytes)
    else:
        data = source.read()
        data_to_print = data[:(len(data)-abs(number_of_bytes))]
    return data_to_print
    
        
def print_lines(Filename,number_of_lines):
# Opens a file, reads the number of lines from the file OR all but the last number of lines.
    data_to_print = ""
    source = open(Filename, 'r')
    if number_of_lines > 0:
        for line in range(0,number_of_lines):
            data_to_print = data_to_print + source.readline()
    else:
        data = source.read().split('\n') # create a list of the lines in the file
        for element in data[:(len(data)-abs(number_of_lines))]:
            data_to_print = data_to_print + element + '\n'
    return data_to_print"""
